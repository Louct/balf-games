...
// Spoof the URL that Unity's Application.absoluteURL reads.
// Only needed when running on a third-party domain — on aetheris.win
// the game files are served locally so no spoof is needed.
(function() {
    const currentHost = window.location.hostname;
    const isOurDomain = currentHost === 'aetheris.win' ||
                        currentHost === 'www.aetheris.win' ||
                        currentHost === 'aetheris.win' ||
                        currentHost === 'www.aetheris.win' ||
                        currentHost === 'localhost' ||
                        currentHost === '127.0.0.1';

    if (isOurDomain) {
        console.log('[SDK] On primary domain, skipping URL spoof.');
        return;
    }

    const fakeOrigin = 'https://tsunamis.io';
    const realPath = window.location.pathname;
    const realSearch = window.location.search;
    const realHash = window.location.hash;
    const fakeHref = fakeOrigin + realPath + realSearch + realHash;

    // Patch document.URL (Unity's Application.absoluteURL uses this)
    try {
        Object.defineProperty(document, 'URL', {
            get: function() { return fakeHref; },
            configurable: true
        });
    } catch(e) {}

    // Patch document.documentURI (fallback Unity may use)
    try {
    ...
    // Patch document.location to return fake values
    try {
    ...
    try {
        window.location.toString = function() { return fakeHref; };
    } catch(e) {}

    console.log('[SDK] URL spoofed to tsunamis.io for third-party host:', currentHost);
})();

// Proxy patch: redirect all tsunamis.io requests through our server
(function() {
	const PROXY_MAP = {
		'https://files.tsunamis.io':   '/tsunamis/files',
		'https://fra-1.tsunamis.io':   '/tsunamis/fra-1',
		'https://fra-2.tsunamis.io':   '/tsunamis/fra-2',
		'https://fra-3.tsunamis.io':   '/tsunamis/fra-3',
		'https://fra-4.tsunamis.io':   '/tsunamis/fra-4',
		'https://fra-5.tsunamis.io':   '/tsunamis/fra-5',
		'https://usa-1.tsunamis.io':   '/tsunamis/usa-1',
		'https://usa-2.tsunamis.io':   '/tsunamis/usa-2',
		'https://usa-3.tsunamis.io':   '/tsunamis/usa-3',
		'https://usa-4.tsunamis.io':   '/tsunamis/usa-4',
		'https://usa-5.tsunamis.io':   '/tsunamis/usa-5',
		'https://asia-1.tsunamis.io':  '/tsunamis/asia-1',
		'https://asia-2.tsunamis.io':  '/tsunamis/asia-2',
		'https://asia-3.tsunamis.io':  '/tsunamis/asia-3',
		'https://asia-4.tsunamis.io':  '/tsunamis/asia-4',
		'https://asia-5.tsunamis.io':  '/tsunamis/asia-5',
	};

	// On third-party domains the URL spoof makes Unity build absolute URLs against
	// https://tsunamis.io for StreamingAssets — we need to proxy those too so they
	// hit /tsunamis/root on balf-games.ink which forwards to the correct local path.
	// On balf-games.ink there is no spoof so Unity uses relative paths — no entry needed.
	const currentHost = window.location.hostname;
	const isOurDomain = currentHost === 'aetheris.win' ||
	                    currentHost === 'www.aetheris.win' ||
	                    currentHost === 'localhost' ||
	                    currentHost === '127.0.0.1';
	if (!isOurDomain) {
		PROXY_MAP['https://tsunamis.io'] = '/tsunamis/root';
	}

	// WebSocket proxy map - routes game server WSS through wsproxy
	const WS_SERVERS = [
		'fra-1.tsunamis.io', 'fra-2.tsunamis.io', 'fra-3.tsunamis.io',
		'fra-4.tsunamis.io', 'fra-5.tsunamis.io',
		'usa-1.tsunamis.io', 'usa-2.tsunamis.io', 'usa-3.tsunamis.io',
		'usa-4.tsunamis.io', 'usa-5.tsunamis.io',
		'asia-1.tsunamis.io', 'asia-2.tsunamis.io', 'asia-3.tsunamis.io',
		'asia-4.tsunamis.io', 'asia-5.tsunamis.io',
	];

	function patchUrl(url) {
		if (typeof url !== 'string') return url;
		for (const [origin, prefix] of Object.entries(PROXY_MAP)) {
			if (url.startsWith(origin)) return url.replace(origin, prefix);
		}
		return url;
	}
	...
	// Patch WebSocket to route game servers through our wsproxy
	const OrigWebSocket = window.WebSocket;
	window.WebSocket = function(url, protocols) {
		if (typeof url === 'string') {
			for (const server of WS_SERVERS) {
				if (url.includes(server)) {
					// Convert wss://fra-2.tsunamis.io:443/path?query
					// to wss://balf-games.ink/wsproxy/fra-2.tsunamis.io:443/path?query
					const parsed = new URL(url);
					const hostPort = parsed.port ? `${parsed.hostname}:${parsed.port}` : parsed.hostname;
					const newUrl = (location.protocol === 'https:' ? 'wss' : 'ws') + '://' + location.host + '/tsunamis/wsproxy/' + hostPort + parsed.pathname + parsed.search;
...
function loadProviderScript(provider) {
    return new Promise((resolve, reject) => {
        ...
        script.src = `/tsunamis/ads/adapters/${provider}-ads.js`;
        script.onload = () => {
            resolve();
        };
        script.onerror = () => {
...
window.SDK = {
    ...
    gameplayStart() {
        if (DEBUG_MODE) console.log("[tsunamis/sdk.js] Gameplay started");
    },
    
    loadingStart() {
        if (DEBUG_MODE) console.log("[tsunamis/sdk.js] Loading started");
        onWindowResize();
    },
    
    loadingEnd() {
        if (DEBUG_MODE) console.log("[tsunamis/sdk.js] Loading finished");
        onWindowResize();
    },
    
    gameplayEnd() {
        if (DEBUG_MODE) console.log("[tsunamis/sdk.js] Gameplay ended");
    },
    ...
    _startTrackingShowtime(adTag) {
        ...
        if (!this._bannerVisibleSince[adTag]) {
            ...
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Started tracking showtime for ${adTag}`);
        }
    },
    ...
    _stopTrackingShowtime(adTag) {
        if (this._bannerVisibleSince[adTag]) {
            ...
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Stopped tracking showtime for ${adTag}. Total showtime: ${Math.floor(this._bannerShowtime[adTag] / 1000)}s`);
        }
    },
    ...
    _resetShowtime(adTag) {
        ...
        delete this._bannerVisibleSince[adTag];
        if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Reset showtime for ${adTag}`);
    },
    ...
    async _originalSetBanner(bannerType, bannerPosition) {
        ...
        if (!adTag || !dims) {
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] SetBanner: Invalid banner type ${bannerType}`);
            return;
        }
        
        if (IS_MOBILE && !dims.enableForMobile) {
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] SetBanner: Banner ${adTag} disabled on mobile`);
            return;
        }
        ...
        // Handle hiding
        if (bannerPosition === 0) {
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Hiding banner ${adTag}`);
    ...
    async _displayBannerWithProviders(bannerType, adTag, container, now) {
        ...
        const tryNextProvider = async () => {
            if (providerIndex >= priorities.length) {
                if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] All providers failed for banner ${adTag} at ${posName}`);
                return false;
            }
            ...
            if (!provider) {
                if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Provider ${providerName} not available for banner ${adTag}`);
                providerIndex++;
                return tryNextProvider();
            }
            
            if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Trying to show banner ${adTag} at ${posName} with provider: ${providerName}`);
            ...
            try {
                ...
                if (success) {
                    if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Successfully showing banner ${adTag} at ${posName} with ${providerName}`);
                    ...
                    if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Showing banner ${adTag} at ${posName} with ${providerName} FAILED, trying next...`);
                    providerIndex++;
                    return tryNextProvider();
                }
            } catch (error) {
                if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Error showing banner ${adTag} with ${providerName}:`, error.message || error);
                providerIndex++;
                return tryNextProvider();
            }
        };
...
window.SDK.SetBanner = function(bannerType, bannerPosition) {
    ...
    if (!window.SDK._isInitialized && bannerPosition !== 0) {
        if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] SDK not initialized yet, queueing banner ${window.bannerMapping[bannerType] || bannerType} for position ${bannerPosition}`);
    ...
    Object.keys(window.SDK._bannerAds).forEach(adTag => {
        ...
        if (window.SDK._isBannerVisible(banner)) {
            ...
            if (currentShowtime >= MIN_REFRESH_INTERVAL) {
                if (DEBUG_MODE) console.log(`[tsunamis/sdk.js] Banner ${adTag} reached ${Math.floor(currentShowtime/1000)}s showtime, refreshing...`);
...
window.SDK._processQueuedBanners = function() {
    ...
    queue.forEach(({ bannerType, bannerPosition }) => {
        if (DEBUG_MODE) {
            ...
            console.log(`[tsunamis/sdk.js] Processing queued banner: ${adTag} at position ${bannerPosition}`);
        }
...
// Initialize
(async function() {
    if (DEBUG_MODE) console.log("[tsunamis/sdk.js] Starting SDK initialization...");
    
    await loadProviders();
    
    // Mark as initialized
    window.SDK._isInitialized = true;
    if (DEBUG_MODE) console.log("[tsunamis/sdk.js] SDK initialized, providers loaded");
