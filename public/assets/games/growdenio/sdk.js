...
// Proxy patch: redirect all growden.io requests through our server
(function() {
	const PROXY_MAP = {
		'https://files.growden.io':  '/proxy/files',
		'https://fra-1.growden.io':  '/proxy/fra-1',
		'https://fra-2.growden.io':  '/proxy/fra-2',
		'https://fra-3.growden.io':  '/proxy/fra-3',
		'https://fra-4.growden.io':  '/proxy/fra-4',
		'https://fra-5.growden.io':  '/proxy/fra-5',
		'https://usa-1.growden.io':  '/proxy/usa-1',
		'https://usa-2.growden.io':  '/proxy/usa-2',
		'https://usa-3.growden.io':  '/proxy/usa-3',
		'https://usa-4.growden.io':  '/proxy/usa-4',
		'https://usa-5.growden.io':  '/proxy/usa-5',
		'https://asia-1.growden.io': '/proxy/asia-1',
		'https://asia-2.growden.io': '/proxy/asia-2',
		'https://asia-3.growden.io': '/proxy/asia-3',
		'https://asia-4.growden.io': '/proxy/asia-4',
		'https://asia-5.growden.io': '/proxy/asia-5',
		'https://growden.io':        '/proxy/growden',
	};

	// WebSocket proxy map - routes game server WSS through Wisp
	const WS_SERVERS = [
		'fra-1.growden.io', 'fra-2.growden.io', 'fra-3.growden.io',
		'fra-4.growden.io', 'fra-5.growden.io',
		'usa-1.growden.io', 'usa-2.growden.io', 'usa-3.growden.io',
		'usa-4.growden.io', 'usa-5.growden.io',
		'asia-1.growden.io', 'asia-2.growden.io', 'asia-3.growden.io',
		'asia-4.growden.io', 'asia-5.growden.io',
	];
	...
	function patchWsUrl(url) {
		if (typeof url !== 'string') return url;
		// Convert wss://fra-2.growden.io:443/path to wss://aetheris.win/wisp/#wss://fra-2.growden.io:443/path
		for (const server of WS_SERVERS) {
			if (url.includes(server)) {
				const wispUrl = (location.protocol === 'https:' ? 'wss' : 'ws') + '://' + location.host + '/wisp/';
				console.log('[SDK] Routing WebSocket through Wisp:', url);
				// Use BareMux/Wisp to proxy the WebSocket
				return url; // Let Wisp handle it via the transport
			}
		}
		return url;
	}
	...
	window.WebSocket = function(url, protocols) {
		if (typeof url === 'string') {
			for (const server of WS_SERVERS) {
				if (url.includes(server)) {
					// Convert wss://fra-2.growden.io:443/path?query
					// to wss://aetheris.win/wsproxy/fra-2.growden.io:443/path?query
					const parsed = new URL(url);
					const hostPort = parsed.port ? `${parsed.hostname}:${parsed.port}` : parsed.hostname;
					const newUrl = (location.protocol === 'https:' ? 'wss' : 'ws') + '://' + location.host + '/wsproxy/' + hostPort + parsed.pathname + parsed.search;
