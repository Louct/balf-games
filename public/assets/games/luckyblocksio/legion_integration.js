...
const GAME_AUTH_URL = IS_PLAYTEST_LINK ? 'https://dev-usa-1.luckyblocks.io/api/accounts/login/legion' : 'https://usa-1.luckyblocks.io/api/accounts/login/legion';
...
const legionSdkInitPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
        if (window.Legion && window.Legion.SDK) {
            ...
            // gameSlug is required for server-authoritative IAP purchases —
            // it tells the Legion API which game's IAP catalog to look the
            // sku up in. The price is then fetched from the server, never
            // from the client. (When embedded inside legionsdk.com/g/[slug],
            // this is auto-provided via the portal handshake, but Luckyblocks
            // runs standalone at luckyblocks.io so we must set it explicitly.)
            window.Legion.SDK.init({ gameSlug: 'luckyblocks-io' });
            ...
            // Listen for all player events from Legion SDK
            if (typeof window.Legion.SDK.player?.onEvent === 'function') {
                window.Legion.SDK.player.onEvent((event, data) => {
                    switch (event) {
                        case 'respawn_request':
                            notifyUnity('LegionListener', 'OnLegionRespawnRequested');
                            break;
                        case 'chat_message_sent':
                            notifyUnity('LegionListener', 'OnLegionChatMessageSent', data);
                            break;
                        case 'pointer_lock_changed':
                            if (data) {
                                notifyUnity('CursorController', 'OnLegionExternalLock');
                            } else {
                                // optional but not needed - cursor.js already handles this. so we are commenting this to avoid a duplicate call.
                                //notifyUnity('CursorController', 'OnLockChange', 'false');
                            }
                            break;
                        default:
                            console.warn(`[LegionIntegration] Unknown player event: ${event}`);
                    }
                });
            } else {
                console.error('[LegionIntegration] player.onEvent not available.');
            }
            ...
            if (typeof window.Legion.SDK.avatar?.onAvatarChanged === 'function') {
            ...
            if (typeof window.Legion.SDK.avatar?.onProportionsChanged === 'function') {
                window.Legion.SDK.avatar.onProportionsChanged((proportions) => {
                    console.log('[LegionIntegration] Proportions changed:', proportions);
                    notifyUnity('LegionListener', 'OnLegionProportionsChanged', JSON.stringify(proportions));
                });
            }
...
/**
 * Validates that the auth result is from the Luckyblocks game server, not raw Legion data.
...
async function authenticateWithGameServer() {
    ...
    authPromise = (async () => {
        try {
            ...
            const username = cachedLegionUser?.username || '';
            window.Legion.SDK.game.loadingStep(`Signing In as ${username}...`);
            ...
            if (result && isValidGameServerResponse(result)) {
                ...
                window.Legion.SDK.game.loadingStep("Signed In! Finding Hub...");
...
/**
 * Report that a player just joined the current room (after you).
 * The SDK checks if they're a friend and shows a "X joined you" toast.
 * 
 * @param {string} username - The player's nickname/username
 */
window.legionPlayerJoined = async function (username) {
    await legionSdkInitPromise;
    window.Legion.SDK.game.playerJoined(username);
};

/**
 * Report that a player was already in the room when you joined.
 * The SDK checks if they're a friend and shows a "You joined X's lobby" toast.
 * 
 * @param {string} username - The player's nickname/username
 */
window.legionPlayerInRoom = async function (username) {
    await legionSdkInitPromise;
    window.Legion.SDK.game.playerInRoom(username);
};
...
wrapLifecycleMethod('loadingStart', () => window.Legion.SDK.game.loadingStep("Loading Started..."));
// For Legion, we stay in LOADING STATE until the hub is joined.
// When index.html/sdk.js calls loadingEnd (Unity WASM loaded), report a step instead of ending.
// The real loadingEnd is called via window.legionLoadingEnd() when the hub is joined.
wrapLifecycleMethod('loadingEnd', () => window.Legion.SDK.game.loadingStep("Game Loaded! Signing in..."));
wrapLifecycleMethod('gameplayEnd', () => window.Legion.SDK.game.gameplayEnd());

// Exposed for C# to report loading steps via jslib
window.legionLoadingStep = async function (message) {
    await legionSdkInitPromise;
    window.Legion.SDK.game.loadingStep(message);
};

// Exposed for C# to signal loading is truly done (hub joined)
window.legionLoadingEnd = async function () {
    await legionSdkInitPromise;
    window.Legion.SDK.game.loadingEnd();
    console.log('[LegionIntegration] loadingEnd called - hub joined');
};
...
// ============================================
// Legion IAP Purchases
// ============================================

/**
 * Purchase an IAP via the Legion SDK (Bux system).
 * Called from Unity C# via WebUtils.ExecuteJS.
 * 
 * @param {string} sku - The Legion IAP SKU (e.g. "gems_500")
 * @param {string} serverBaseUrl - The current game server base URL for webhook routing (e.g. "https://fra-2.luckyblocks.io")
 */
window.legionPurchase = async function (sku, serverBaseUrl) {
    await legionSdkInitPromise;
    console.log(`[LegionIntegration] legionPurchase: sku=${sku}, serverBaseUrl=${serverBaseUrl}`);

    try {
        const metadata = {};
        if (serverBaseUrl) {
            metadata.webhookUrl = `${serverBaseUrl}/api/legion-webhook`;
        }

        const result = await window.Legion.SDK.bux.requestPurchase(sku, metadata);
        console.log('[LegionIntegration] Purchase result:', JSON.stringify(result));

        notifyUnity('IAPManager', 'OnLegionPurchaseResult', JSON.stringify({
        ...
        console.error('[LegionIntegration] Purchase error:', error);
        notifyUnity('IAPManager', 'OnLegionPurchaseResult', JSON.stringify({
            success: false,
            sku: sku,
            transactionId: '',
