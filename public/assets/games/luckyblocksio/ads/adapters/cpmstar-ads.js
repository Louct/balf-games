...
(function () {
    ...
    log("[cpmstar-ads.js] Module loading...");
    ...
    function initCpmstarVideo() {
        log("[cpmstar-ads.js] Initializing CPMStar video ads...");
    ...
    function initCpmstarAPI() {
        log("[cpmstar-ads.js] CPMStar API callback");

        if (typeof cpmstarAPI !== 'undefined') {
            cpmstarAPI(function (api) {
                log("[cpmstar-ads.js] Setting up CPMStar...");
                ...
                cpmstarAPI({
                    ...
                    onAdOpened: function () {
                        log("[cpmstar-ads.js] Interstitial opened");
                    },
                    onAdClosed: function () {
                        log("[cpmstar-ads.js] Interstitial closed");
                    },
                    fail: function () {
                        log("[cpmstar-ads.js] Interstitial failed");
                ...
                cpmstarRewardedVideo.addEventListener("ad_opened", function (e) {
                    log("[cpmstar-ads.js] Rewarded video opened");
                });

                cpmstarRewardedVideo.addEventListener("ad_closed", function (e) {
                    log("[cpmstar-ads.js] Rewarded video closed");
                ...
                cpmstarRewardedVideo.addEventListener("loaded", function (e) {
                    log("[cpmstar-ads.js] Rewarded video loaded");
                });

                cpmstarRewardedVideo.addEventListener("load_failed", function (e) {
                    log("[cpmstar-ads.js] Rewarded video failed to load");
                });

                // Preload first rewarded video
                cpmstarRewardedVideo.load();
                cpmstarInitialized = true;
                log("[cpmstar-ads.js] Initialization complete");
            });
        }
    }
    ...
    function triggerCpmstarBanner(placementId, element) {
        log(`[cpmstar-ads.js] Triggering banner for placement ID: ${placementId}`);
        ...
        log(`[cpmstar-ads.js] Elements found: ${els.length}, using index: ${pindex}`);
        ...
        log(`[cpmstar-ads.js] Banner command queued`);
    }

    // Video ad provider implementation
    window.videoAdProviders.cpmstar = {
        showMidroll: function (onSuccess, onFailure) {
            log(`[cpmstar-ads.js] showMidroll called`);

            if (typeof cpmstarAPI !== 'undefined') {
                cpmstarAPI({
                    kind: "game.displayInterstitial",
                    onAdOpened: function () {
                        log(`[cpmstar-ads.js] Midroll opened`);
                    },
                    onAdClosed: function () {
                        log(`[cpmstar-ads.js] Midroll closed`);
                        onSuccess();
                    },
                    fail: function () {
                        log(`[cpmstar-ads.js] Midroll failed`);
                        onFailure();
                    }
                });
            } else {
                log(`[cpmstar-ads.js] API not available`);
                onFailure();
            }
        },
        showRewarded: function (onSuccess, onFailure) {
            log(`[cpmstar-ads.js] showRewarded called`);
            ...
            if (cpmstarRewardedVideo && cpmstarRewardedVideo.isLoaded()) {
                ...
                log(`[cpmstar-ads.js] Rewarded video not loaded`);
                cpmstarRewardedVideo.load();
                onFailure();
            } else {
                log(`[cpmstar-ads.js] Rewarded video not initialized`);
                onFailure();
            }
        }
    };
    ...
    window.bannerAdProviders.cpmstar = {
        displayBanner: async function (bannerType, container) {
            ...
            log(`[cpmstar-ads.js] displayBanner for type: ${bannerType}, placement ID: ${placementId}`);

            if (!placementId) {
                log(`[cpmstar-ads.js] No placement ID for banner type ${bannerType}`);
                return false;
            }
            ...
            if (!dims) {
                log(`[cpmstar-ads.js] No dimensions for banner type ${bannerType}`);
                return false;
            }
            ...
            const success = await new Promise((resolve) => {
                ...
                if (!window.cpmstarBannerScriptLoaded) {
                    log(`[cpmstar-ads.js] Loading banner script`);
                    const script = document.createElement('script');
                    script.async = true;
                    script.src = 'https://ssl.cdne.cpmstar.com/cached/js/lib.js';
                    script.onload = function () {
                        log(`[cpmstar-ads.js] Banner script loaded`);
                    ...
                    script.onerror = function () {
                        log(`[cpmstar-ads.js] Failed to load banner script`);
                        clearTimeout(timeoutId);
                        observer.disconnect();
                        settle(false);
                    };
    ...
    log("[cpmstar-ads.js] Module loaded successfully");
})();
