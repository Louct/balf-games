...
(function () {
    ...
    log("[adinplay-ads.js] Module loading...");

    // AdinPlay tag mapping - can be customized per game
    const ADINPLAY_TAG_MAPPING = {
        "300x250": "luckyblocks-io_300x250",
        "728x90": "luckyblocks-io_728x90",
        "300x600": "luckyblocks-io_300x600"
    };
    ...
    function initAdinPlay() {
        log("[adinplay-ads.js] Initializing AdinPlay...");
        ...
        aiptag.cmd.player.push(function () {
            log("[adinplay-ads.js] Creating AdinPlay player...");
            aiptag.adplayer = new aipPlayer({
                ...
                PREROLL_ELEM: function () {
                    ...
                    log(`[adinplay-ads.js] Preroll element: ${elem ? 'found' : 'not found'}`);
                    return elem;
                },
                AIP_COMPLETE: function (state) {
                    log(`[adinplay-ads.js] Video Ad Completed: ${state}`);
                    ...
                    if (isFailure) {
                        ...
                        log("[adinplay-ads.js] Ad succeeded");
            ...
            log("[adinplay-ads.js] AdinPlay player created");
        });

        // Load the AdinPlay script
        log("[adinplay-ads.js] Loading AdinPlay script...");
        const adinplayScript = document.createElement('script');
        adinplayScript.type = "text/javascript";
        adinplayScript.src = '//api.adinplay.com/libs/aiptag/pub/LGP/luckyblocks.io/tag.min.js';
        adinplayScript.async = true;
        adinplayScript.onload = function () {
            log("[adinplay-ads.js] Script loaded successfully");
        };
    ...
    window.videoAdProviders.adinplay = {
        showMidroll: function (onSuccess, onFailure) {
            log(`[adinplay-ads.js] showMidroll called`);
            ...
            if (typeof aiptag !== 'undefined' && typeof aiptag.adplayer !== 'undefined') {
                ...
                aiptag.cmd.player.push(function () {
                    log(`[adinplay-ads.js] Starting preroll`);
                    aiptag.adplayer.startPreRoll();
                });
            } else {
                log(`[adinplay-ads.js] API not available`);
                onFailure();
            }
        },
        showRewarded: function (onSuccess, onFailure) {
            log(`[adinplay-ads.js] showRewarded called (delegates to showMidroll)`);
    ...
    window.bannerAdProviders.adinplay = {
        displayBanner: async function (adTag, container) {
            log(`[adinplay-ads.js] displayBanner for tag: ${adTag}`);

            // Map generic adTag to AdinPlay-specific tag
            const adinplayTag = ADINPLAY_TAG_MAPPING[adTag];
            if (!adinplayTag) {
                log(`[adinplay-ads.js] No mapping found for adTag: ${adTag}`);
                return false;
            }
            log(`[adinplay-ads.js] Using AdinPlay tag: ${adinplayTag}`);
            ...
            if (!(isReady && typeof aiptag !== "undefined" && typeof aipDisplayTag !== "undefined")) {
                log(`[adinplay-ads.js] Not available for banner`);
                return false;
            }
    ...
    log("[adinplay-ads.js] Module loaded successfully");
})();
