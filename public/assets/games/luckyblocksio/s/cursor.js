...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isLegionEmbedded = new URLSearchParams(window.location.search).has('legionsdk');
  const cursorMap = {};

  if (isLegionEmbedded) {
    const CURSOR_CDN = 'https://static.legionsdk.com/cursors';
    cursorMap['default'] = `url('${CURSOR_CDN}/default.png') 4 0, auto`;
    cursorMap['pointer'] = `url('${CURSOR_CDN}/pointer.png') 10 0, pointer`;
  }

  function setCanvasCursor(value) {
    canvas.style.setProperty('cursor', value, isLegionEmbedded ? 'important' : '');
  }

  function resolveCursor(style) {
    return cursorMap[style] || style;
  }
  ...
  function onPointerLockChange() {
    ...
    if (locked) {
      ...
      setCanvasCursor(resolveCursor("default"));
    }
  ...
  window.setCursor = function setCursor(cursorStyle) {
    if (canvas) {
      setCanvasCursor(resolveCursor(cursorStyle));
    }
  };
});
