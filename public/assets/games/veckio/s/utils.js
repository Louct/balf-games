// Spoof document.URL so Unity host-check sees veck.io
(function() {
  try {
    // Patch document.URL (configurable unlike window.location)
    Object.defineProperty(document, 'URL', {
      configurable: true,
      get: function() { return 'https://veck.io/'; }
    });
    // Patch document.documentURI as well
    ...
    // Patch eval to intercept Unity's JS_Eval_EvalJS host check
    const _eval = window.eval;
    window.eval = function(code) {
      if (typeof code === 'string' && code.includes('window.location.href')) {
        code = code.replace(/window\.location\.href/g, '"https://veck.io/"');
      }
      return _eval.call(this, code);
    };
    console.log('[utils] document.URL spoof installed');
  } catch(e) {
    console.warn('[utils] spoof failed:', e);
  }
})();
...
function sendPopState() {
  if (typeof unityInstance === 'undefined' || !unityInstance) return;
