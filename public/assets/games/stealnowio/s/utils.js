...
window.addEventListener('beforeunload', function (e) {
  ...
  // Save guest data before closing
  try {
    if (window.unityInstance) {
      unityInstance.SendMessage('NetworkManager', 'SaveGuestDataFromJS');
    }
  } catch (err) {
    console.error('[beforeunload] Failed to save guest data:', err);
  }
...
function sendPopState() {
  if (typeof unityInstance !== 'undefined' && unityInstance !== null && unityInstance.SendMessage) {
    unityInstance.SendMessage('BrowserHistoryManager', 'PopState', window.location.pathname);
  } else {
    console.log('[Utils] Unity instance not ready yet, skipping PopState');
  }
}
